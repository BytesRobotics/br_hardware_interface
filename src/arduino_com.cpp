...
bool HardwareCom::set_controller(double rmotor_cmd, double lmotor_cmd, double head_cmd)
{
  ...
  for (int i = 1; i < outgoing_packet_length_ - 1; i++) {
    outgoing_packet_[outgoing_packet_length_ - 1] ^= outgoing_packet_[i];
  }
...
//        std::cout << "\n";
    ...
    for (int i = 1; i < incoming_packet_length_ - 1; i++) {
      pec ^= incoming_packet_[i];
    }
...
//            std::cout << "PEC Correct\n";
      ...
      for (int i = 0; i < 2 * num_channels_; i += 2) {
        channels_[i / 2] = static_cast<int>(incoming_packet_[i] | (incoming_packet_[i + 1] << 8));
      }

      auto o = 2 * num_channels_;     //! offset
      encoder_left_ =
        static_cast<int>(incoming_packet_[o] | (incoming_packet_[o + 1] << 8) |
        (incoming_packet_[o + 2] << 16) | (incoming_packet_[o + 3] << 24));
      encoder_right_ =
        static_cast<int>(incoming_packet_[o + 4] | (incoming_packet_[o + 5] << 8) |
        (incoming_packet_[o + 6] << 16) | (incoming_packet_[o + 7] << 24));
      ...
      altitude_ = static_cast<int>(incoming_packet_[o + 24] | (incoming_packet_[o + 25] << 8)) /
        100.0;
...
int HardwareCom::get_dist_sensor_value(DistSensor sensor)
{
  ...
  int returnVal;
  if (abs(last_channels_[channel] - channels_[channel]) < max_diff_ ||
    channels_last_update_skipped_[channel])
  {
